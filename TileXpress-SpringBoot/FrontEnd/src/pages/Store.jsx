...
function Store() {
  ...
  const handleFilter = (value, sectionId) => {
    ...
    if (sectionId === "category") {
      ...
      if (updatedCategoryFilter.length > 0) {
        searchParams.set(sectionId, updatedCategoryFilter.join(","))
      } else {
  ...
  const fetchFilterData = async ({ data }) => {
    const categoryQuery = data.category.length > 0 ? data.category.join(",") : ""
    await axios
      .get(
        `http://localhost:8081/products?category=${categoryQuery}&size=${data.size}&minPrice=${data.minPrice}&maxPrice=${data.maxPrice}&minDiscount=${data.minDiscount}&sort=${data.sort}&stock=${data.stock}&pagenumber=0&pageSize=10&color=${data.color.join(",")}`,
        {},
      )
      .then((res) => {
      ...
      <div>
        ...
        <main className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="flex items-baseline justify-between border-b border-gray-200 pb-6 pt-8">
            <h1 className="text-xl lg:text-4xl font-bold tracking-tight text-gray-900">New Arrivals</h1>
                  ...
                  .map((item) => (
                    ...
                    >
                      ...
                      <div className="w-full max-h-fit px-4 py-3">
                        <h3 className="text-sm mb-0.5 font-medium text-gray-900">{item.title}</h3>
                        <p className="text-sm mb-1 font-light text-gray-900">{item.brand}</p>
                        <p className="text-sm">
                          ₹{item.discountedPrice}
                          <s className="mx-3 text-zinc-500">₹{item.price}</s>
                          <span className="text-green-500">{item.discountedPercent}% off</span>
                        </p>
                      </div>
                    </Link>
                  ))}
